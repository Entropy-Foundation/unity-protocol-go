...
type Encoder interface {
	...
	Join(dst io.Writer, shards [][]byte, outSize int) (error error, response []byte)
}
...
func (r reedSolomon) Join(dst io.Writer, shards [][]byte, outSize int) (error error, response []byte) {
	// Do we have enough shards?
	if len(shards) < r.DataShards {
		return ErrTooFewShards, nil
	}
	...
	for _, shard := range shards {
		if shard == nil {
			return ErrReconstructRequired, nil
		}
		size += len(shard)
	...
	write := outSize
	for _, shard := range shards {
		// fmt.Println("\n-----------------------ROUND-START-REEDSOLOMON-------------------------------------------------------", response)
		// fmt.Println("\nVALUES_OF_WRITE", write)
		// fmt.Println("\nVALUES_OF_OUTSIZE", outSize)
		// fmt.Println("\nMAIN:",  shard)
		if write < len(shard) {
			// _, _ = dst.Write(shard[:write]) // write to file
			// fmt.Println("\nFIRST-SHARD:", shard[:write])
			response = append(response, shard[:write]...)
			// fmt.Println("\nResponse", response)
			// fmt.Println("\nError", err)
			return nil, response
		...
		// fmt.Println("\nVALUES_OF_N_FROM_WRITE_FUNCTION:", n)
		// fmt.Println("\nSECOND-SHARD:", shard)
		...
		// }
		write -= len(shard)
		// fmt.Println("\n------------------------ROUND-END-REEDSOLOMON--------------------------------------------------------", response)
	}
	// return nil,shards
