...
// func PairingFromBytes(bytes []byte) (Pairing, error) {
// 	s := C.CString(string(bytes))
// 	defer C.free(unsafe.Pointer(s))
// 	pairing := (*C.struct_pairing_s)(C.malloc(sizeOfPairing))
// 	rv := C.pairing_init_set_str(pairing, s)
// 	if rv == 1 {
// 		return Pairing{}, errors.New("bls.FromBytes: Failed to create Params from bytes.")
// 	}
// 	return Pairing{pairing}, nil
// }
...
func (system System) PublicKeyToBytes(publicKey PublicKey) []byte {
	...
	bytes := make([]byte, n)
	C.element_to_bytes_compressed((*C.uchar)(unsafe.Pointer(&bytes[0])), publicKey.gx.get)

	return bytes
}

func (system System) PrivateKeyToBytes(privateKey PrivateKey) []byte {
	n := int(C.pairing_length_in_bytes_compressed_G1(system.pairing.get))
	if n < 1 {
		return nil
	}
	...
	bytes := make([]byte, n)
	C.element_to_bytes((*C.uchar)(unsafe.Pointer(&bytes[0])), privateKey.x.get)

	return bytes
}

func (system System) PublicFromBytes(bytesData []byte) (PublicKey, error) {
	...
	x := (*C.struct_element_s)(C.malloc(sizeOfElement))
	C.element_init_G2(x, system.pairing.get)
	C.element_from_bytes_compressed(x, (*C.uchar)(unsafe.Pointer(&bytesData[0])))

	// x := (*C.struct_element_s)(C.malloc(sizeOfElement))
	// C.element_init_G2(x, system.pairing.get)
	// C.element_from_hash(x, unsafe.Pointer(&bytesData[0]), sha256.Size)

	return PublicKey{system, Element{x}}, nil

}

// func (system System) PrivateFromBytes2(bytesData []byte) (PublicKey, error) {
...
func (system System) PrivateFromBytes(bytesData []byte) PrivateKey {
	n := int(C.pairing_length_in_bytes_compressed_G1(system.pairing.get))
	...
	return PrivateKey{system, Element{x}}
}
...
// func (pairing Pairing) ToBytes() []byte {
// 	var buf *C.char
// 	var size C.size_t
...
// 	C.pbc_param_out_str(f, pairing.get)
// 	C.fclose(f)
